...
const OrderItem = ({ id, quantity, name, price, handleClickOpen }) => {
...
export const OrderPage = () => {
    ...
    (async () => {
      ...
      try {
        for (let item of dummyData) {
          awaitArr.push(axios.get(`/api/v1/item/${item.id}/`));
        }
        let responses = await Promise.allSettled(awaitArr);
        for (let res of responses) {
          if (res.status === "fulfilled" && res.value?.data?.length > 0)
            result.push(res.value.data[0])
        }
        if (valid) {
          setData(result)
        }
  ...
  const handleSubmit = async () => {
    await axios.post("/api/v1/placeorder/", { "menu-items": data.map((item) => item.id) });
  ...
  return <Container>
    ...
    {data.map((item) => (
      <OrderItem id={item.id} price={item.price} name={item.name} quantity={1} handleClickOpen={handleClickOpen} key={`order-item-${item.id}`} />
    ))}
    <Dialog
      open={open}
      onClose={handleClose}
    ...
    <Grid item textAlign={"end"} mt={8}>
      ...
      <Typography fontSize={19}>$ {totalPrice}</Typography>
    </Grid>
