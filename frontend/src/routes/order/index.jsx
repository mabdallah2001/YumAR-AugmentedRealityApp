...
import { clearOrder, deleteFromOrder } from "../../main";
...
export const OrderPage = () => {
    ...
    (async () => {
      ...
      try {
        for (let id of orderData) {
          awaitArr.push(axios.get(`/api/v1/item/${id}/`));
        }
  ...
  const handleSubmit = async () => {
    await axios.post("/api/v1/placeorder/", {
      "menu-items": data.map((item) => item.id),
    });
    clearOrder();
  ...
  return (
    <Container>
      ...
      >
        ...
        <DialogContent>
          <DialogContentText id="alert-dialog-description">
            Are you sure you want to delete this menu item? This is an
      ...
      <Grid item textAlign={"end"} mt={8}>
        ...
        <Typography fontSize={19}>$ {totalPrice.toFixed(2)}</Typography>
      </Grid>
